...
import {
  ...
  MessageSquare, Activity, Globe, Send, Zap, CalendarDays
} from "lucide-react";
...
import PartenaireJobDating from "./partenaire/PartenaireJobDating";
    ...
    <div className="space-y-6 animate-fade-in" data-testid="partenaire-dashboard">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-slate-100 border border-slate-200 flex-wrap h-auto gap-1 p-1">
          ...
          <TabsTrigger value="jobdating" data-testid="tab-jobdating"><CalendarDays className="w-4 h-4 mr-1.5" /> Job Dating</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="jobdating" className="space-y-4 mt-4">
          <PartenaireJobDating token={token} />
        </TabsContent>
      </Tabs>
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <Card className="border-dashed border-2 border-slate-200"><CardContent className="flex flex-col items-center justify-center py-12">
          <Users className="w-12 h-12 text-slate-300 mb-3" /><p className="text-slate-500 text-sm">Aucun bénéficiaire</p>
        </CardContent></Card>
      ) : (
...
// ===== D'CLIC PRO PROFILE CARD (for Profil tab) =====
const DclicProProfileCard = ({ profile }) => {
  const dp = profile.dclic_profile || {};
  const ia = dp.integrated_analysis || {};
  const n1 = ia.niveau_1_preuves?.elements || {};
  const n2 = ia.niveau_2_fonctionnement?.elements || {};
  const n3 = ia.niveau_3_regulation?.elements || {};
  const riasec = dp.riasec_profile || {};
  const vertus = dp.vertus_profile || {};
  const compass = dp.compass || {};
  const ofman = dp.ofman_quadrant || [];

  return (
    <Card className="border-l-4 border-l-indigo-500 border border-indigo-100 mt-4" data-testid="dclic-pro-profile-card">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          <Brain className="w-5 h-5 text-indigo-600" /> Profil D'CLIC PRO
        </CardTitle>
        <CardDescription>Résultats du test psychométrique D'CLIC PRO — autorisé par le bénéficiaire</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {compass.summary && (
          <div className="bg-indigo-50 rounded-lg p-3">
            <p className="text-xs font-semibold text-indigo-700 mb-1">Boussole intérieure</p>
            <p className="text-sm text-indigo-800">{compass.summary}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="bg-amber-50 rounded-lg p-3">
            <p className="text-xs font-semibold text-amber-700 mb-1.5">Motivations</p>
            <div className="flex flex-wrap gap-1.5">
              {(dp.motivations || []).map((m, i) => <Badge key={i} className="bg-amber-100 text-amber-800 text-xs">{m}</Badge>)}
            </div>
            {n3.moteur_ennea?.moteur && <p className="text-xs text-amber-600 mt-1.5">Moteur : {n3.moteur_ennea.moteur}</p>}
            {n3.moteur_ennea?.evitement && <p className="text-xs text-amber-500 mt-0.5">Évitement : {n3.moteur_ennea.evitement}</p>}
          </div>
          <div className="bg-purple-50 rounded-lg p-3">
            <p className="text-xs font-semibold text-purple-700 mb-1.5">Style professionnel (DISC)</p>
            {n2.style_disc && (
              <>
                <p className="text-sm text-purple-800 font-medium">{n2.style_disc.style}</p>
                {n2.style_disc.force_principale && <p className="text-xs text-purple-600 mt-1">Force : {n2.style_disc.force_principale}</p>}
                {n2.style_disc.besoin && <p className="text-xs text-purple-500 mt-0.5">Besoin : {n2.style_disc.besoin}</p>}
              </>
            )}
          </div>
        </div>

        {/* Compétences fortes & Forces */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {(profile.dclic_competences || dp.competences_fortes || []).length > 0 && (
            <div>
              <p className="text-xs font-semibold text-slate-600 mb-1.5">Compétences fortes</p>
              <div className="flex flex-wrap gap-1.5">
                {(profile.dclic_competences || dp.competences_fortes || []).map((c, i) => (
          ...
          {(n1.forces_cles || []).length > 0 && (
            <div>
              <p className="text-xs font-semibold text-slate-600 mb-1.5">Forces clés</p>
              <div className="flex flex-wrap gap-1.5">
                {n1.forces_cles.map((f, i) => <Badge key={i} variant="outline" className="text-xs text-green-700 border-green-300">{f}</Badge>)}
              </div>
            </div>
          )}
        </div>
        ...
        {/* Vigilances */}
        {(profile.dclic_vigilances || dp.vigilances || []).length > 0 && (
          <div className="bg-orange-50 rounded-lg p-3">
            <p className="text-xs font-semibold text-orange-700 mb-1.5">Points de vigilance</p>
            <div className="flex flex-wrap gap-1.5">
              {(profile.dclic_vigilances || dp.vigilances || []).map((v, i) => (
                <Badge key={i} variant="outline" className="text-xs text-orange-700 border-orange-300">{v}</Badge>
              ))}
            </div>
          </div>
        )}
        ...
        {(n1.conditions_reussite || []).length > 0 && (
          <div>
            <p className="text-xs font-semibold text-slate-600 mb-1.5">Conditions de réussite</p>
            <div className="flex flex-wrap gap-1.5">
              {n1.conditions_reussite.map((c, i) => <Badge key={i} variant="outline" className="text-xs">{c}</Badge>)}
            </div>
          </div>
        )}
        ...
        {(n2.environnement_favorable || []).length > 0 && (
          <div>
            <p className="text-xs font-semibold text-slate-600 mb-1.5">Environnement de travail favorable</p>
            <div className="flex flex-wrap gap-1.5">
              {n2.environnement_favorable.map((e, i) => <Badge key={i} className="bg-teal-50 text-teal-700 text-xs">{e}</Badge>)}
            </div>
          </div>
        )}
        ...
        {ofman.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-slate-600 mb-2">Quadrant d'Ofman</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {ofman.map((q, i) => (
                <div key={i} className="border border-slate-200 rounded-lg p-2.5 text-xs">
                  <p className="font-semibold text-green-700 mb-0.5">Qualité : {q.qualite}</p>
                  <p className="text-red-600">Piège : {q.piege}</p>
                  <p className="text-blue-600">Défi : {q.defi}</p>
                  <p className="text-slate-500 mt-1 italic">{q.source}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {ia.synthese && (
          <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
            <p className="text-xs font-semibold text-slate-600 mb-1">Synthèse D'CLIC PRO</p>
            <p className="text-sm text-slate-700">{ia.synthese}</p>
          </div>
        )}
      </CardContent>
    </Card>
    ...
    <div className="space-y-4" data-testid="beneficiaire-detail">
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab}>
        ...
        <TabsContent value="profil" className="mt-4">
          ...
          {/* D'CLIC PRO Section in Profil */}
          {linkedProfile?.profile?.dclic_imported && (
            <DclicProProfileCard profile={linkedProfile.profile} />
          )}
        </TabsContent>
    ...
    <div className="space-y-4" data-testid="observatoire-contribution">
      <Card className="border border-slate-100">
        ...
        <CardContent className="space-y-4">
          ...
          {lastContribution && <p className="text-xs text-green-600">Dernière contribution envoyée avec succès !</p>}
        </CardContent>
      </Card>
    </div>
  );
        ...
        <div className="space-y-4 py-2">
          ...
          {searched && results.length === 0 && !searching && (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-slate-400 mt-1">Le bénéficiaire doit avoir : identifiant France Travail renseigné, mode "Partenaire à l'insertion professionnelle" activé, test D'CLIC passé et profil boosté</p>
            </div>
          )}
