...
- [x] (2026-08) REVUE DE CODE appliquée (iteration_51, 36/36 tests OK, aucune régression) : (1) secret en dur POST /admin/gate-state → env ADMIN_GATE_PASSWORD (backend/.env) ; (2) 17 défauts mutables `body: dict = {}` → `Body(default_factory=dict)` (server.py x15, dclic_routes.py x2) ; (3) secrets retirés des tests : tests/conftest.py (TEST_USER_PASSWORD depuis .env), 21 fichiers migrés + admin_gate ; (4) opc/seed.py random.Random(42) = démo déterministe volontaire (commenté, faux positif sécurité) ; (5) points "variables non définies" et "is vs ==" de la revue = FAUX POSITIFS (flake8 F821/F632 = 0). DÉFÉRÉ AU BACKLOG (risque/effort élevés) : refactor register_dclic_routes (431 lignes), split server.py (87 imports, déjà P0), type hints >80%, refactors dclic_scoring. ⚠️ Au prochain déploiement vérifier que ADMIN_GATE_PASSWORD existe en prod.
