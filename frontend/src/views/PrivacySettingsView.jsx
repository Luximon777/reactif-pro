...
const PrivacySettingsView = ({ token }) => {
  ...
  const savePrivacy = async () => {
    if (visibilityLevel === "limited" && (!realFirstName.trim() || !realLastName.trim())) {
      toast.error("Nom et prénom requis pour le mode Partenaire à l'insertion professionnelle");
      return;
    }
    setSaving(true);
    try {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const res = await axios.get(`${API}/auth/export-data?token=${token}`, { responseType: "blob" });
      const url = URL.createObjectURL(res.data);
      const a = document.createElement("a");
      a.href = url;
      a.download = `reactif-pro-export-${new Date().toISOString().split("T")[0]}.txt`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success("Document exporté : profil, trajectoire et rapport D'CLIC PRO");
    } catch (err) {
  ...
  const visibilityOptions = [
    ...
    { value: "limited", label: "Partenaire à l'insertion professionnelle", desc: "Accessible aux partenaires de parcours autorisés. Votre nom et prénom seront communiqués.", icon: Eye },
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700 mt-1">
                    En choisissant le mode "Partenaire à l'insertion professionnelle", vous acceptez que votre nom, prénom et identifiant France Travail soient communiqués aux partenaires de parcours (Mission Locale, France Travail, APEC, etc.) qui effectuent une demande d'accès à votre profil. Vos données restent protégées et le partage est révocable à tout moment.
                  </p>
                </div>
              </div>
