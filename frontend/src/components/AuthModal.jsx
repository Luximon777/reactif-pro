...
const AuthModal = ({ open, onOpenChange, defaultRole = "particulier", onSuccess }) => {
  ...
  const isVsi = defaultRole === "vsi";

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[460px] p-0 overflow-hidden" data-testid="auth-modal">
        {/* Header */}
        <div className={`${isVsi ? "bg-amber-600" : "bg-[#1e3a5f]"} px-6 pt-6 pb-5`}>
          <DialogHeader>
            <DialogTitle className="text-white text-xl" style={{ fontFamily: 'Outfit, sans-serif' }}>
              {tab === "login" ? "Connexion" : "Créer un compte"}
              {isVsi && <span className="text-amber-200 text-base ml-2">— Parcours VSI</span>}
            </DialogTitle>
          </DialogHeader>
          <p className={`${isVsi ? "text-amber-200" : "text-blue-200"} text-sm mt-1`}>
            {isVsi ? "Accompagnement pour valoriser votre identité professionnelle" : "Espace personnel confidentiel sous pseudonyme"}
          </p>
          {/* Tab Switcher — hidden for VSI (login only) */}
          {!isVsi && (
          ...
          {isVsi && (
            <p className="text-amber-200/80 text-xs mt-3 italic">
              Connectez-vous avec vos identifiants RE'ACTIF PRO existants
            </p>
          )}
        </div>
        ...
        <div className="p-6">
          {(!isVsi && tab === "register") ? (
            ...
            <LoginForm onSuccess={onSuccess} loginPseudo={loginPseudo} targetRole={defaultRole} />
          )}
        </div>
      </DialogContent>
    </Dialog>
...
const LoginForm = ({ onSuccess, loginPseudo, targetRole }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    setLoginError(false);
    const result = await loginPseudo(pseudo, password, targetRole);
    if (result.success) {
