              ...
              <table className="w-full text-xs" data-testid="opc-referentiel-table">
                ...
                <tbody>
                  {opcResults.map((r, idx) => {
                    ...
                    // Hard Skills: use hard_skills, fallback to capacites_techniques (shortened)
                    const hardSkills = (r.hard_skills || []).length > 0 ? r.hard_skills : (r.capacites_techniques || []).map(c => typeof c === "string" && c.length > 60 ? c.substring(0, 60) + "…" : c);
                    // Soft Skills: use soft_skills, fallback to ck1 comp cognitives + emotionnelles
                    const softSkills = (r.soft_skills || []).length > 0 ? r.soft_skills : [...(r.ck1_comp_cognitives || []).slice(0, 3), ...(r.ck1_comp_emotionnelles || []).slice(0, 2)];
                    return (
